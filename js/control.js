	...
	if (window['balances'] === undefined) {
		...
		for (var i = 0; i < Keys.length; i++) {
			if (parseFloat(Balance[Keys[i]].available) != 0 || parseFloat(Balance[Keys[i]].onOrder) != 0) {
				...
				row += `    		<span class='pull-right' id='Amount-ETH'>${Balance[Keys[i]].available}</span><br>`;
				row += `    		<span class='pull-right text-yellow' id='Amount-Lock-ETH'>${Balance[Keys[i]].onOrder}</span>`;
				row += "    	</span>";
				row += "    </h4>";
				row += "  </a>";
				row += "</li>";
...
function AddAutoRow(Coin, Amount) {
	...
	row += '<li>';
	row += '	<a>';
	row += `		<h4 class="control-sidebar-subheading auto-coin" symbol="${Coin}" amount="${Amount}">`;
